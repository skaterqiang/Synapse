...
| 领域模板 | 模板管理（实体/概念类型、提取规则、页面骨架）、AI 一键生成、图谱抽取时领域选择与自动建模 |
...
| 作业 | 并发作业队列、阶段状态机、中断/恢复/重试，作业管理页实时查看进度与子任务输出 |
...
4. 图谱抽取前选择领域：🤖 自动匹配已有领域（未命中会在作业内自动归纳新建）、📚 指定已有领域、✨ 自动创建新领域或 ✍️ 手动新建领域
5. 处理进度在「作业管理」页查看，图谱在「知识图谱管理」区块查看
