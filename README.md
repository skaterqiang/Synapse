...
| 存储 | SQLite（sql.js）持久化：知识数据使用 `knowledge.db`，应用 API Key、MCP 与 Skills 配置使用独立的 `app.db`；统一数据根目录可配置迁移；笔记为 Markdown 文件，git 友好 |
...
> **重要：** 切勿将 API Key 硬编码到源码中。API Key、MCP 与 Skills 配置通过应用设置界面存入独立的 `<数据根目录>/app.db`，不会写入知识库 `knowledge.db`。
...
| 数据 | 位置 |
|---|---|
| 知识库数据库（目录 / 图谱 / 领域模板 / 作业历史 / 问答会话与收藏） | `<数据根目录>/knowledge.db` |
