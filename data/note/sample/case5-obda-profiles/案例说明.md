...
本笔记对应 protege-js 示例 `case5-obda-profiles.js`，通过 OWL 2 RL 推理展示业务场景。
...
| 步骤 | 做什么 | 对应目录内容 / 入口 |
|---|---|---|
| **① 客户上传资料 → 按顶层本体体系构建图谱** | 把「资料/」下的业务文档作为来源，选定体系后抽取本体层 | 资料：`资料/01-多源系统集成方案.md`、`资料/02-设备主数据台账.md`<br>体系：**`iso15926`**（在提取弹窗显式指定，即五级优先级链的第 ① 级） |
...
node scripts/run-sample-cases.js case5-obda-profiles
```
