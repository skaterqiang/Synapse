...
本笔记对应 protege-js 示例 `case2-family-swrl.js`，通过 OWL 2 RL 推理展示业务场景。
...
| 步骤 | 做什么 | 对应目录内容 / 入口 |
|---|---|---|
| **① 客户上传资料 → 按顶层本体体系构建图谱** | 把「资料/」下的业务文档作为来源，选定体系后抽取本体层 | 资料：`资料/01-家庭成员登记.md`、`资料/02-亲属关系规则.md`<br>体系：**`bfo-lite`**（在提取弹窗显式指定，即五级优先级链的第 ① 级） |
...
node scripts/run-sample-cases.js case2-family-swrl
```
