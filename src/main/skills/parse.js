...
const { MINERU_IMAGE_EXTS, MAX_IMAGE_BYTES, PER_SKILL_CHARS, TOTAL_SKILL_CHARS } = require('../common/constants');
...
// 预算常量已上提到 common/constants.js（语料流水线的 SkillMarkdownDecorator 复用同一口径），
// 此处仅保留别名以便本文件内阅读：值与历史完全一致，未做任何调整
...
// 抽成独立函数是为了让语料流水线的 SkillMarkdownDecorator 复用**同一段**取内容逻辑
// （语料流水线设计 §5.3：mode: llm 的抽取技能与现状 parseWithSkills 只差「注入哪些技能」一处）。
// 返回 { ok:true, content } 或 { ok:false, error }
async function buildUserContent(absPath, ext, buffer, opts = {}) {
  const content = [];
  if (MINERU_IMAGE_EXTS.has(ext)) {
    ...
    content.push({ type: 'image_url', image_url: { url: `data:${mime};base64,${buffer.toString('base64')}` } });
    content.push({ type: 'text', text: `以上是图片文件 ${path.basename(String(absPath))} 的内容，请解析并以 Markdown 输出。` });
  ...
  const cap = num(opts.settings, 'sourceMaxChars', 60000, 1000, 1000000);
  let body = text;
  let truncated = false;
...
// opts: { buffer, builtinText, builtinExtract(ext, buffer) -> Promise<string>, onLog(line), skills }
//   builtinText：调用方已提取的内置文本（优先使用，避免重复解析）；
//   builtinExtract 由 files.js 注入（避免本模块反向依赖解析层）：未提供文本时用它取；
//   skills：显式指定参与的技能（语料流水线传 selectExtractSkills 的结果）；缺省 = 全部已启用技能（现状行为）
async function parseWithSkills(absPath, settings, opts = {}) {
  const ext = path.extname(String(absPath)).toLowerCase();
  const skills = Array.isArray(opts.skills) ? opts.skills.filter((k) => k && k.name) : enabledSkills(settings);
  ...
  const built = await buildUserContent(absPath, ext, buffer, { ...opts, settings });
...
module.exports = { skillParseReady, enabledSkills, hasModel, skillInstructions, buildSystemPrompt, cleanOutput, buildUserContent, parseWithSkills };
