...
> 分支：`protege` ｜ 状态：v0.1 草案（**纯设计，未实现**） ｜ 日期：2026-09-15
>
...
// meta：加工痕迹，每层追加
{
  ...
  corpusFile:  'corpus/充电桩扩容/供电容量说明.md', // 语料文件相对路径（CorpusWriteDecorator）
...
module.exports = {
  ...
  loadIndex(), saveIndex(),           // index.json
  staleOf(rec),                       // 源文件 mtime 变了 → stale:true（复用 raws.js:59 isIngestedFresh 的判据口径）
};
```

`index.json` 单条形态（**11 字段**）：

```js
{ corpusId, rel, name, domain, domainLabel, profileId, parseMethod, skill, chars, generatedAt, sourceMtime }
```

> 索引也可放 SQLite（`db.js` 已有 kv 与 jobs 表）。选 JSON 文件的理由：与 `note/` 的文件式存储一致、可被外部工具直接读、损坏时可从 `corpus/**/*.md` 的 frontmatter 全量重建。**待拍板**（§15 开放问题 2）。
...
| `corpus/index.json` 单条 | **11** | 同上 | 新契约（§6.3） |
...
| `corpus/` 索引损坏 | `store.js:loadIndex` | 从 `corpus/**/*.md` 的 frontmatter 全量重建 | 一次性 toast |
...
> **本文档是纯设计交付物。截至 2026-09-15（commit `6c94da9`），下表全部能力均未实现。** 判断实现状态一律以本表为准，不要读 §3–§9 的代码骨架就以为已落地（那些是**目标态**描述）。
...
## 15. 开放问题（6 项已拍板 / 2 项待定）

> **拍板记录（2026-09-15，用户裁定）**：问题 **1 / 1b / 4 / 5 / 6 / 7** 共 6 项已定案，结论已回写至 §5.3、§7.4、§10.1、§13、§16.3、§16.9 与 §6.1；问题 **2 / 3** 仍开放（问题 3 依赖外部文档 `提取去重判断方案.md`）。
...
| **2** | 语料索引存 JSON 文件还是 SQLite | A. `corpus/index.json`（与 `note/` 文件式存储一致，可外部读，损坏可重建）<br>B. SQLite 新表（事务安全，与 `jobs` 表同构） | ❓ **仍开放**。建议 **A**：语料是可重生成的机器产物，索引损坏的代价是「扫一遍目录」，不值得引入表结构迁移 |
