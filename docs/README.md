...
> Synapse 是一款本地优先的个人知识库助手（Electron 桌面应用）。它把「Markdown 笔记 → 原始资料 → 结构化 Wiki → 知识图谱 → AI 问答」串成一条完整的知识加工流水线，所有数据保存在本机。
...
| [3. AI 问答](03-AI问答.md) | 多数据源问答、会话管理、思考过程、澄清问题、技能与 MCP |
| [4. 原始文件管理](04-原始文件管理.md) | 添加本机文件/目录、引用式管理、吸收状态 |
| [5. LLM Wiki](05-LLM-Wiki.md) | 吸收来源、浏览 Wiki、Wiki 问答、体检报告 |
| [6. 领域模版](06-领域模版.md) | 模版结构、AI 生成、吸收前自动匹配与建模 |
| [7. 知识图谱](07-知识图谱.md) | 抽取本体、图谱视图、实体浏览、本体定义、KG 问答 |
| [8. 作业管理](08-作业管理.md) | 作业队列、阶段状态、子任务、重试与并发 |
| [9. 提示词管理](09-提示词管理.md) | 11 类系统提示词的查看与自定义 |
| [10. 设置](10-设置.md) | 模型配置、存储、作业、问答、编辑器、MCP、技能 |
...
理解这 5 个概念，就掌握了 Synapse 的设计思路：

| 概念 | 说明 |
|---|---|
| **笔记（Note）** | 你手写的 Markdown 内容，存为 `<数据根目录>/note/` 下的 `.md` 文件，可直接用其他编辑器打开 |
| **原始文件（Raw）** | 待加工的原始资料（PDF/DOCX/网页等）。本机文件采用**引用式**管理，不复制副本 |
| **LLM Wiki** | AI 把原始资料「编译」成的结构化知识页面，遵循 OKF v0.2 规范（Markdown + YAML frontmatter），按 `<领域>/<类型>/` 组织 |
| **领域模版（Domain Template）** | 告诉 AI「这个领域该抽取什么」的规则集（实体类型、概念类型、必须提取/忽略内容、页面骨架） |
| **知识图谱（Knowledge Graph）** | AI 从笔记与 Wiki 中抽取的实体与关系网络，可视化展示并支持基于图谱的问答 |

## 典型工作流

```
① 收集资料              ② 定义领域规则           ③ AI 加工
原始文件页添加文件  →   领域模版（可 AI 生成）  →  右键「提取 Wiki」
                                                  右键「提取知识图谱」
                                                        ↓
④ 后台执行                                        ⑤ 使用知识
作业管理页看进度    ←─────────────────────────→   Wiki 阅读 / 图谱浏览
（可重试、看报告）                                 AI 问答 / KG 问答
```

> **重要操作约定**：所有「提取 Wiki / 提取知识图谱」的入口统一为**右键菜单**。在原始文件行、目录、笔记卡片上右键即可触发。
...
| LLM Wiki 页面（Markdown 文件） | `<数据根目录>/llmwiki/` |
...
本手册的全部截图均由 `scripts/capture-docs.js` 从**实际运行的应用**自动采集（1680×1050 窗口，Retina 3024×1694）。需要更新截图时：
...
| AI 问答面板 | [ai-chat.png](images/ai-chat.png) |
| Wiki 阅读 | [wiki-reader.png](images/wiki-reader.png) |
| 吸收新来源弹窗 | [wiki-ingest-dialog.png](images/wiki-ingest-dialog.png) |
